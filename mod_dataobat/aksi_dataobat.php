...
$kodeobat=$_GET['id'];
	if($_GET['module']=='tambah'){
		...
		header("location:../media.php?module=data_obat");
	
	}
	elseif($_GET['module']=='hapus'){		
		$query=mysql_query("DELETE FROM  tbyr where  no_kwitansi='$kodeobat'");
		$query1=mysql_query("DELETE FROM  tdetailbyr where  no_kwitansi='$kodeobat'");
		header("location:../media.php?module=data_obat");		
	}
